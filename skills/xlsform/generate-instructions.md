...
| `name`              | Short English variable name of lowercase letters and digits only, ≤ 20 chars, e.g. `jobsatisfaction` (no umlauts, no underscores except the `_other` suffix of a Sonstiges follow-up).                                                                                                       |
...
| `researchQuestions` | Array of the numbers of the research questions this question serves, e.g. `[1]` or `[1, 3]`. Research questions are numbered from 1 in the order given.                                                                                                                                      |
| `relevant`          | Only for questions that not everyone should see: the condition on an earlier answer, e.g. `${usage} = 'yes'` or `selected(${areas}, 'other')`. Omit it otherwise.                                                                                                                            |
...
- Check the qwac question bank **before** writing questions from scratch. If `questionBank` is given, it lists the bank questions that matched a keyword search for this research goal (demographics excluded); pick from it and don't search again. Otherwise use the qwac MCP tools: `search_questions` matches substrings, so search **one keyword at a time** (e.g. `Zufriedenheit`, not `Zufriedenheit Homeoffice`), **at most 3 searches**, then write the rest yourself
...
- Add `"Keine Angabe"` as a choice for sensitive questions. In a `select_multiple`, mark it (and any "Nichts davon") with `exclusive: yes`, so it can't be ticked together with other answers; in a `select_one` it is exclusive anyway, so leave the flag out
- **One question, one thing** (chapter „Fragen formulieren", rule 4: no double stimuli). A label must not ask about two things joined by „und"/„oder", e.g. „Wie bewerten Sie die Kommunikation und den Informationsfluss?". Split it into two questions
- **Filter questions** (chapter „Fragebogenaufbau", „Filterführung"): a question that only applies to some respondents gets a `relevant` on the earlier answer that decides it. „In welchen Bereichen haben Sie … angewendet?" only for those who applied it; a question „… wenn Sie … nicht …" only for those who didn't. If no earlier question decides it, add that filter question first
- **Sonstiges:** add a choice with code `other` and label „Sonstiges", then directly after the question a `text` follow-up named `<question>_other` with `relevant` `selected(${<question>}, 'other')` for `select_multiple`, `${<question>} = 'other'` for `select_one`. Don't use `or_other`
- **Yes/no follow-ups** („Falls ja, …"): directly after the yes/no question, with `relevant` `${<question>} = '<yes code>'`
- Fill `reasoning` with a short German account of how the set of questions was arrived at: which research question and construct is covered by which block, what was taken from the question bank and what was written from scratch. The user sees this text — it is the questionnaire's audit trail. If a research question is too broad, leading, or can't be answered with a survey, say so briefly here as a hint (don't refuse)
...
Add per-language columns `label::<lang>`, `hint::<lang>`, etc., where `<lang>` is an IETF BCP-47 tag (`de`, `en`, `fr-BE`), bare or in parentheses after a name (`label::Deutsch (de)`). Set `default_language` on the `settings` sheet the same way.

LimeSurvey has a fixed set of language codes. A regional tag it lacks falls back to its language with a warning (`fr-BE` → `fr`); a language it lacks entirely (e.g. `eo`), or two tags that land on one code (`fr` and `fr-BE`), fails the LimeSurvey conversion. DDI keeps every tag as written.
