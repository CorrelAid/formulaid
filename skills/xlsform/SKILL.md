...
Skill version: f8ef7553 (formtransform v0.2.0)
...
| File                                                                                           | Contents                                                                                                                                                                                                     |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [cdl-survey-types/SKILL.md](cdl-survey-types/SKILL.md)                                         | Authoritative question-type catalogue and XLSForm syntax allowlist (vendored from `CorrelAid/formtransform`). The registry is an allowlist — unregistered types are _rejected_ downstream, not approximated. |
| [cdl-survey-types/references/question-types.md](cdl-survey-types/references/question-types.md) | Per-type detail: XLSForm `type` string, constraints, variants, worked example.                                                                                                                               |
| [cdl-survey-types/references/xlsform-syntax.md](cdl-survey-types/references/xlsform-syntax.md) | Allowed XLSForm syntax: naming rules, choice sheet, `or_other`, skip/validation logic, appearances, `settings` sheet, metadata rows.                                                                         |
| [references/survey-methodology.md](references/survey-methodology.md)                           | Survey methodology: question formulation, structure, DSGVO, code patterns, quality checklist. No XLSForm specification — that lives in `cdl-survey-types/`.                                                  |
...
Skip unless the user signals analytical intent (_Zusammenhang, Unterschied, vergleichen, aufschlüsseln nach, Subgruppen_) or explicitly names demographic variables. If skipped, include minimal defaults: age as integer, gender.
...
Additional essentials (full XLSForm syntax in [cdl-survey-types/references/xlsform-syntax.md](cdl-survey-types/references/xlsform-syntax.md); methodology in [references/survey-methodology.md](references/survey-methodology.md)):
